...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
  ...
  */
    ...
    /* USB and SAI DMA share the highest preemption level so stream-control
       resets cannot interrupt an in-progress FIR state update. */
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  //USB用内部メモリ確保
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData , 0x01 , PCD_DBL_BUF, 0x01600098);

  //FeedBack Endpoint確保
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData , 0x82 , PCD_SNG_BUF, 0x0280);
